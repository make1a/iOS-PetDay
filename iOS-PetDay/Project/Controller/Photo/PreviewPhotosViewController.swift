...
extension PreviewPhotosViewController:UICollectionViewDelegate,UICollectionViewDataSource {
     ...
     func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
       ...
       let imageCell = PreviewPhotosCell.dequeueReusable(withCollectionView: collectionView, for: indexPath)
        ...
        imageCell.photoImageView.hero.id = "image_\(indexPath.item)"
        imageCell.photoImageView.hero.modifiers = [.position(CGPoint(x:view.bounds.width/2, y:view.bounds.height+view.bounds.width/2)), .scale(0.6), .fade]
        return imageCell
    }
